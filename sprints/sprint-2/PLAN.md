...
> 更新时间：2026-05-02（排程经理第417轮，5 CC活跃；全部CC稳定运行；PR#4425/#4426/#4427 mergeState=UNKNOWN待关注）
...
| 2 | ~~#1497~~ | P2 | backend | ~~NBO评分引擎 — 项目适配度×意图×赢率综合评分（kimi2已接）~~ | 🔥 |
| 3 | ~~#1538~~ | P2 | budget | ~~D3造价回填接口 — D3设计完成后自动写入设备科目预算（kimi3已接）~~ | 🔥 |
| 4 | ~~#1539~~ | P2 | budget | ~~历史基准自动积累 — 项目关闭时自动更新成本基准库（kimi4已接）~~ | 🔥 |
| 5 | ~~#1590~~ | P2 | budget | ~~项目成本复盘报告自动生成（kimi5已接）~~ | 🔥 |
| 6 | #1597 | P2 | budget | 员工借款额度校验Service | ✅ |
| 7 | #1601 | P2 | budget | 报销申请接入预算关卡 | ✅ |
| 8 | #1818 | P2 | brand | 平台定制发布 — 多平台标题/标签/封面微调 | ✅ |
| 9 | #1824 | P2 | brand | 内容创作/编辑页面 — 富文本+多媒体上传+双语编辑 | ✅ |
...
| 14 | #1770 | P2 | frontend | 新增权限管理页面 — 用户管理+部门管理+模块注册+操作日志 | ✅ |
| 15 | #1843 | P2 | design-ai | 招标规范文本自动生成 — 三标融合+产品参数→投标技术文本 | ✅ |
| 16 | #1880 | P2 | asset-library | 素材库 - 中标状态变更时自动更新素材权限 | ✅ |
| 17 | #1882 | P2 | asset-library | 素材库 - 列表查询 + 多维度筛选 + 权限过滤 | ✅ |
| 18 | #1941 | P2 | design-ai | 文生图代理API — Spring Boot封装G7e + 外部API双轨Fallback | ✅ |
| 19 | #2013 | P2 | backend | 敏感字段组+权限模板 — 一键分配岗位权限 | ✅ |
| 20 | #2021 | P2 | approval | 报销费控模块接入审批流 — 报销提交自动发起审批 | ✅ |
...
|------|-------|------|--------|------|--------|
| kimi1 | #1496 | backend | P2 | NBO行动推荐生成 — Today's Top 3行动 | 31% 🔥（启动后212min） |
| kimi2 | #1497 | backend | P2 | NBO评分引擎 — 项目适配度×意图×赢率综合评分 | 48% 🔥（启动后212min） |
| kimi3 | #1538 | budget | P2 | D3造价回填接口 — D3设计完成后自动写入设备科目预算 | 66% 🔥（启动后212min） |
| kimi4 | #1539 | budget | P2 | 历史基准自动积累 — 项目关闭时自动更新成本基准库 | 🔍 PR#4427 REVIEW_REQUIRED |
| kimi5 | #1590 | budget | P2 | 项目成本复盘报告自动生成 | 39% 🔥（启动后212min） |

> **5 CC活跃，池子已满**；kimi1 31%（持平）、kimi2 48%（持平）、kimi3 66%（持平）、kimi4 **已提PR#4427**、kimi5 39%（持平）；PR#4415/4421/4422/4423/4424已merged；PR#4425/#4426/#4427 mergeState=UNKNOWN
