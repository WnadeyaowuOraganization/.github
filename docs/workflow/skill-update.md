...
- 来源：kimi1 #2339 CC-REVIEW（PR #4488）
- 状态：**待处理**（排程经理不处理环境脚本，登记后移交研发经理）
...
- 止血：注入修正指令，要求用 `gh pr list --search "#3378" in:title` 精确搜索
- 预防：前端 coding skill 应强调搜索 PR 时加 `in:title` 限定，避免模糊匹配误导
