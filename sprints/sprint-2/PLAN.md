...
> 活跃CC：1个（#1911 AI电池包开发助手），Todo队列22项，建议P1优先
...
> 更新时间：2026-05-01（排程经理第297轮，kimi1-kimi4已接，kimi5待指派）
> ⚠️ needs-prototype冻结、Master Issue不出现在本表
> 注意：本表与GitHub Issue编号一致，无需转换

| # | Issue | 优先级 | 模块 | 说明 | 启动 |
|---|-------|--------|------|------|------|
| 1 | #2045 | P1 | backend | AI生成Phase7 — 方案配图自动生成 | ✅ |
| 2 | #2037 | P1 | backend | D3-优化[10/10] — 改造/翻新模式（已有结构约束下的设计） | ✅ |
| 3 | ~~#2163~~ | P1 | backend | ~~素材库API+RBAC权限管理（kimi1已接）~~ | 🔥 |
| 4 | ~~#2126~~ | P1 | fullstack | ~~经销商发现 — 修正版种子数据+方法论后端（kimi2已接）~~ | 🔥 |
| 5 | ~~#2031~~ | P1 | backend | ~~色卡材料Phase5 — 非标色卡/材料使用审批流程（kimi3已接）~~ | 🔥 |
| 6 | ~~#4124~~ | P1 | fullstack | ~~国贸专属页 — 壳架构+5Tab容器+菜单入口（kimi4已接）~~ | 🔥 |
| 7 | ~~#2747~~ | P1 | backend | ~~客户生命周期[4/12] 客户满意度回访提醒（kimi1已完成，PR#4391 merged）~~ | ✅ |
| 6 | ~~#2737~~ | P1 | backend | ~~商战情报中台·分发[2/8] 企微交互式查询指令（kimi1已完成，PR#4394 merged）~~ | ✅ |
| 7 | ~~#3188~~ | P1 | fullstack | ~~回款资料[4/7] 企业信息库 — 万德常用信息维护+一键填充到回款表单（kimi2已完成，PR#4393手动合并）~~ | ✅ |
| 8 | ~~#3208~~ | P1 | backend | ~~总控预算增强[1/10]（kimi2已完成，PR#4392 merged）~~ | ✅ |
| 9 | ~~#3570~~ | P1 | frontend | ~~关系网络Tab组件测试与截图验证（kimi5已完成，PR#4388 merged）~~ | ✅ |
| 10 | ~~#2738~~ | P1 | backend | ~~商战情报中台·分发[3/8] 邮件订阅与定时报告（kimi3已完成，PR#4390 merged）~~ | ✅ |
| 11 | ~~#1978~~ | P0 | backend | ~~方案引擎DB设计 — 方案模板引擎3张表（kimi1已完成，PR#4395 merged）~~ | ✅ |
| 12 | ~~#1974~~ | P1 | pipeline | ~~方案引擎DB设计 — 素材库3张表（kimi1已完成，PR#4396 merged）~~ | ✅ |
| 13 | ~~#1961~~ | P1 | backend | ~~方案引擎AI排版引擎后端API — 版式分析+智能布局+一键优化（kimi2已完成，PR#4398 merged）~~ | ✅ |
| 14 | ~~#1969~~ | P1 | backend | ~~方案引擎COM Add-in核心 — Ribbon界面+JS桥接+后端通信协议（kimi3已完成，PR#4397 merged）~~ | ✅ |
| 15 | ~~#1965~~ | P2 | backend | ~~方案引擎PowerPoint加载项集成测试（kimi3已完成，PR#4399 merged）~~ | ✅ |
| 16 | ~~#1966~~ | P1 | backend | ~~PPT插件样式一致性检查+一键修复（kimi2已完成，PR#4375 merged）~~ | ✅ |
| 17 | ~~#1970~~ | P0 | backend | ~~方案引擎PPT插件后端API（kimi3已完成，PR已提交）~~ | ✅ |
| 18 | ~~#1959~~ | P1 | backend | ~~方案引擎PPT插件：文字美化工具（kimi1已完成，PR#4400 merged）~~ | ✅ |
| 19 | ~~#1960~~ | P1 | backend | ~~方案引擎PPT插件：AI排版工具箱（kimi2已完成，PR#4401已提交）~~ | ✅ |
...
|------|-------|------|--------|------|--------|
| kimi1 | #2163 | backend | P1 | 素材库API+RBAC权限管理 | 49% 编码中 |
| kimi2 | #2126 | fullstack | P1 | 经销商发现种子数据+方法论后端 | 55% 编码中 |
| kimi3 | #2031 | backend | P1 | 色卡材料审批流程（#2032已CLOSED） | 54% 编码中 |
| kimi4 | #4124 | fullstack | P1 | 国贸专属页 — 壳架构+5Tab容器+菜单入口 | PR #4376 BLOCKED（质量预检❌，修复中） |
| kimi5 | #2045 | backend | P1 | AI生成Phase7: 方案配图自动生成 | 新指派（#4045 Master已kill） |
