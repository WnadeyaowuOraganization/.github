...
| Issue 类型 | 必经 skill |
|-----------|-----------|
| 纯后端 CRUD | issue-task-md → cc-report → backend-schema → backend-coding → backend-test（**按顺序 JUnit → Playwright API spec 两门必过**，curl 仅 debug）→ cc-report → pr-visual-proof（无图）→ cc-report |
...
11. **禁止无测试的 PR**：后端改动 feature 分支 diff 必须**按顺序**包含 JUnit 单测（先过，含编译验证）+ **Playwright API spec**（后跑，HTTP 契约 + 鉴权 + DB 落库端到端，取代 curl smoke 作证据门）；前端改动必须含 Playwright e2e spec；Bug 修复必含"复现红灯"测试；纯文档 / 配置 Issue 必须在 task.md 显式注明"跳过测试原因"
