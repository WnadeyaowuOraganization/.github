...
> 更新时间：2026-05-09（排程经理第1127轮；17个Plan→Todo同步入表；清空旧已完成条目）
> ⚠️ needs-prototype冻结、Master Issue不出现在本表；指派前请用 `gh issue view #N` 确认标签
> 注意：本表与GitHub Issue编号一致，无需转换

| # | Issue | 优先级 | 模块 | 说明 | 启动 |
|---|-------|--------|------|------|------|
| 1 | #2371 | P1 | fullstack | 竞品技术浏览器 — 产品对标/参数查询/CAD下载（Master #4065 Phase7） | ✅ |
| ~~2~~ | ~~#3195~~ | P1 | backend | 计划模板引擎 — 中标后一键生成项目主计划（project-plan [3/10]） | ✅ 已指派 kimi2 |
| 3 | #3198 | P1 | frontend | 项目主计划页面 — 条线视图+甘特图（project-plan [6/10]） | ✅ |
| ~~4~~ | ~~#2020~~ | P2 | approval | 色卡材料审批迁移 — 从独立审批迁移到通用引擎（Phase15 [15/22]） | ✅ 已指派 kimi8 |
| ~~5~~ | ~~#2286~~ | P2 | budget | 采购申请预算校验前端集成（补充1） | ✅ CLOSED |
| ~~6~~ | ~~#2335~~ | P2 | sample | 样品借还+样品箱总览看板（Phase15 [15/16]） | ✅ 已指派 kimi8 |
| ~~7~~ | ~~#2313~~ | P2 | frontend | 图生图独立GUI页面 — 上传+选项+生成，零对话交互（设计模块 [3/30]） | ✅ 已指派 kimi8 |
| ~~8~~ | ~~#2331~~ | P2 | fullstack | 文生图页面 — 双模式输入+4并发预览+历史画廊（Phase5 [5/6]） | ✅ 已指派 kimi8 |
| 9 | #2413 | P2 | pipeline | 竞品出口客户反向采集脚本 — 凯奇/奇特乐/永浪海外买家（海外客户发现 [2/3]） | ✅ |
| ~~10~~ | ~~#2414~~ | P2 | pipeline | 海关数据游乐设备进口商采集脚本 — Trademo/公开数据（海外客户发现 [1/3]） | ✅ 已指派 kimi7 |
| ~~11~~ | ~~#2433~~ | P2 | backend | 项目链条时间窗口预测 — 设计→施工→采购（[28/38]） | ✅ 已指派 kimi5 |
| ~~12~~ | ~~#2443~~ | P2 | backend | ICP画像构建 → 矿场评分模型校准（[14/38]） | ✅ 已指派 kimi8 |
| ~~13~~ | ~~#2448~~ | P2 | backend | 前期项目分频次定期复验机制（[8/38]） | ✅ 已指派 kimi8 |
| ~~14~~ | ~~#2449~~ | P2 | pipeline | 中标公告自动提取所有中标企业 → 新竞品自动发现（[7/38]） | ✅ 已指派 kimi5 |
| 15 | #2451 | P2 | backend | 竞品中标项目名称分词 → 补充keyword_pool搜索盲区（[5/38]） | ✅ |
| ~~16~~ | ~~#2456~~ | P2 | design-ai | 训练数据自动Caption标注 — Qwen3.5-VL批量生成结构化描述（Phase1 [3/14]） | ✅ 已指派 kimi8 |
| ~~17~~ | ~~#2093~~ | P3 | backend | AI增强验收 — 智能检查清单+报告生成+智能派单（执行管理） | ✅ 已指派 kimi8 |
| 18 | #2118 | P3 | execution | AI BOM智能推荐 — 历史学习+智能匹配+价格估算（执行管理） | ✅ |
| 19 | #2142 | P3 | brand | 竞品内容监测 — MediaCrawler定时采集+竞品动态展示（品牌中心） | ✅ |
| 20 | #2143 | P3 | brand | AI内容建议 — 标题优化+标签推荐+最佳发布时间（品牌中心） | ✅ |
