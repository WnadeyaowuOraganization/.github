...
> ⏰ 最后更新：2026-04-04 02:32 by Perplexity Computer
...
- 执行管理（57个Issue）：数据库建表→CRUD API→图纸/BOM/采购/生产/安装/验收/变更/开票/安全→利润/成本/回款→AI预警→EVM挣值管理→前端页面
- CRM（6个）：跟进记录+商机管理+经销体系+报价引擎
- 数据迁移（5个）：明道云→执行管理迁移
- 品牌中心（3个）：多平台数据采集+竞品监测
- 其他（5个）：质保/协同/代理商/提成
