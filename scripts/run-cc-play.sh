...
if [ -f "$PARSER" ]; then
  tmux new-session -d -s "$SESSION" -c "$WORK_DIR" \
    "cd $WORK_DIR && claude -p '拾取并完成 Issue #${ISSUE}' --model $MODEL --output-format stream-json 2>&1 | tee $RAW_LOG | python3 $PARSER 2>&1 | tee -a $LOGFILE"
else
  tmux new-session -d -s "$SESSION" -c "$WORK_DIR" \
    "cd $WORK_DIR && claude -p '拾取并完成 Issue #${ISSUE}' --model $MODEL --output-format text 2>&1 | tee -a $LOGFILE"
fi
