...
|------|-------|------|--------|------|--------|
| kimi1 | #1496 | backend | P2 | NBO行动推荐生成 — Today's Top 3行动 | 28% ⚠️（启动后804min，进度回退31%→28%） |
| kimi2 | #1497 | backend | P2 | NBO评分引擎 — 项目适配度×意图×赢率综合评分 | 48% 🔥（启动后804min） |
| kimi3 | #1538 | budget | P2 | D3造价回填接口 — D3设计完成后自动写入设备科目预算 | 66% 🔥（启动后804min） |
| kimi4 | #1539 | budget | P2 | 历史基准自动积累 — 项目关闭时自动更新成本基准库 | 43% 🔥（启动后804min） |
| kimi5 | #1590 | budget | P2 | 项目成本复盘报告自动生成 | 49% 🔥（启动后789min） |

> **5 CC活跃，池子已满**；kimi1进度回退31%→28%，疑似上下文切换代价或代码回退；其他4 CC持平；PR#4415/4421/4422/4423/4424已merged；PR#4427/4425/4426 REVIEW_REQUIRED
