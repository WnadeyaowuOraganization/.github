...
| D89 | 04-17 | ✅ | Sprint1 测试修复期 + Skill体系信息同步盲区识别 + 后端先于前端约束 | **Sprint1 截止**：2026-04-20（周一）正式交付；04-17~19 为测试+修复期，禁止派发新 Issue，当前5CC（kimi1~7，含临时超额）跑完即止，Sprint2 Issue（执行管理/S3管线/全过程资料/制度管理）全部暂缓。**G7e完全停机确认（D84补充）**：D84记录G7e停机但18个S3数据管线 Issue 中描述仍引用G7e；2026-04-17 已向全部10个S3管线 Issue（#3290~#3299）评论+issue-source.md追加备注「G7e已停机，m7i接管」。**后端先于前端派发约束（新规）**：同一页面功能，后端 Issue 必须先 merged 前端才可派发，防止前端以 mock 数据交付；已同步到：`scheduler-guide.md`排序规则#6、`assign-guide.md` Step 3 前置确认、`frontend-coding/SKILL.md` 删除 mock 允许条款。**单Issue单CC指派规则（新规）**：指派建议表每行只能对应一个 kimi + 一个 Issue，禁止合并派发（防止研发经理将多 Issue 合并到同一 CC）。**#3292 bot 错误关闭修复**：PR#3828（#3301前端页面）错误关闭了 #3292（加密文件检测Python脚本，从未实现）；研发经理已 reopen，Sprint-2 补排。**Prompt→Skill 信息同步盲区**：D85a skill化后，规约更新只写 SKILL.md，不再广播给所有经理会话；关键规则变更后需主动用 tmux send-keys 通知活跃 CC，否则经理会话内存储的旧规则不会自动刷新。 | 排程经理 |
