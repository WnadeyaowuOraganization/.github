...
> 更新于 2026-04-20（全批PLM剩余Issue一次性列出，研发经理按CC槽分批派发）
> 当前运行5/5：kimi1/#3385 + kimi3/#3651 + kimi4/#3383 + kimi6/#3384 + kimi19/#3388

### ★ 立即可启动（前置全部Done）

| 建议kimi | Issue | 优先 | 内容 | 前置状态 |
|---------|-------|------|------|---------|
| 空槽 | **#3394** | P1 | PLM[17/20] 零件主数据管理页面（frontend） | 前置#3378✅ |
| 空槽 | **#3395** | P1 | PLM[18/20] BOM管理与版本对比页面（frontend） | 前置#3380✅#3381✅ |

### ◎ 等待当前CC完成后可启动

| 建议kimi | Issue | 优先 | 内容 | 前置 | 等待谁 |
|---------|-------|------|------|------|-------|
| 空槽 | **#3386** | P0 | PLM[9/20] ECO审批流集成（backend，复用审批引擎） | #3385 | kimi1/#3385 |
| 空槽 | **#3389** | P1 | PLM[12/20] 超级BOM→实例EBOM解析API | #3388 | kimi19/#3388 |
| 空槽 | **#3392** | P1 | PLM[15/20] 版本化定价引擎API（价格锚定BOM版本） | #3383,#3380✅ | kimi4/#3383 |
| 空槽 | **#3387** | P0 | PLM[10/20] ECO执行引擎（审批通过→版本更新） | #3386 | #3385+#3386链 |
| 空槽 | **#3390** | P1 | PLM[13/20] D3参数化→PLM BOM桥接 | #3389 | #3388+#3389链 |
| 空槽 | **#3391** | P1 | PLM[14/20] 产品配置器后端API（超级BOM规则过滤） | #3388,#3389 | #3388+#3389链 |
| 空槽 | **#3393** | P1 | PLM[16/20] 供应商价格联动（报价→Part Master→BOM重算） | #3392 | #3392链 |
| 空槽 | **#3396** | P1 | PLM[19/20] ECO变更管理页面（frontend） | #3385,#3386 | #3385+#3386链 |
| 空槽 | **#3397** | P1 | PLM[20/20] 产品配置器+报价页面（frontend） | #3391 | #3391链 |

> **派发原则**：
> - 后端（backend）优于前端（frontend）
> - PLM[9/20]#3386 完成后立即派 #3387；#3388完成后立即派 #3389；#3383完成后立即派 #3392
> - #3394/#3395（frontend）可在任意空槽时启动，与backend并行无冲突
> - 每批尽量并行：#3386+#3389+#3392 可3CC同时运行（无冲突）
