...
> 更新时间：2026-05-06（排程经理第863轮；kimi3#1616 PR#4454 MERGED释放，补#2434→Todo，4CC运行中）
...
| # | Issue | 优先级 | 模块 | 说明 | 启动 |
|---|-------|--------|------|------|------|
| 1 | #1698 | P2 | backend | 提成绩效新增提成计算引擎API — 阶梯计算+绩效系数+批量计算 | ✅ |
| 2 | #2073 | P2 | hr | 人事管理菜单和权限SQL — 人事管理菜单+按钮权限配置（kimi2已提PR#4463） | ⏳ PR#4463 CI=FAILURE |
| 3 | #1838 | P2 | design-ai | 批量方案变体生成 — 输入约束→AI生成多个布局方案 | ✅ |
| 4 | #1842 | P2 | design-ai | 产品资源开放体系 — D1素材库2D/3D/SKP下载API | ✅ |
| 5 | #2100 | P2 | backend | AI工单智能分派 — 自动推荐最优处理人 | ✅ |
| 6 | #2100 | P2 | backend | AI工单智能分派 — 自动推荐最优处理人 | ✅ |
| 7 | #2101 | P2 | backend | AI预测性维护 — 故障预测+预防性维护计划 | ✅ |
| 8 | #2133 | P1 | backend | 企微OA能力扩展 — 审批+日程+会议+智能表格+模板卡片 | ✅ |
| 9 | #2144 | P2 | brand | AI辅助翻译 — Qwen3中英互译 | ✅ |
| 10 | #2147 | P2 | pipeline | G7e工具链部署 — PRD Generator + Acceptance Tester | ✅ |
| 11 | #2200 | P1 | brand | G7e MediaX SDK — 抖音/B站/小红书/YouTube发布 | ✅ |
| 12 | #2166 | P1 | backend | AI图生图API — 图片分析+出图引擎（新入Todo） | ✅ |
| 13 | #2420 | P1 | backend | Agent基建 CLAUDE.md + docs/agent-guide.md（新入Todo） | ✅ |
| 14 | #2426 | P1 | pipeline | llm_client.py — vLLM统一调用封装（新入Todo） | ✅ |
| 15 | #2382 | P1 | hr | 前端培训计划+课程库页面 — 计划管理+课程CRUD+课件上传（新入Todo） | ✅ |
| 16 | #2336 | P1 | sample | D3样品一键生成页面（新入Todo） | ⏳ 依赖后端前置Phase |
| 17 | #2442 | P1 | backend | Lookalike搜索模式 → 同类项目跨区域复制（新入Todo） | ✅ |
| 18 | #2438 | P1 | backend | 投标人名单提取 → 发现同场竞技者（新入Todo） | ✅ |
| 19 | #2436 | P1 | pipeline | 财政预算信号采集 → 六层信号源第三层（新入Todo） | ✅ |
| 20 | #2434 | P1 | backend | 信号链自动串联 → 同项目不同阶段信号关联（新入Todo） | ✅ |
...
| kimi2 | #2073 | hr | P2 | 培训管理菜单和权限SQL | ⏳ PR#4463 质量预检FAIL |
...
> **5 CC运行中**（pool 10/15空闲）；#1616 PR#4454 MERGED✅，kimi3释放→#2340；#4463质量预检FAIL修复中；#4464新PR CI跑中；kimi1(74%)需关注上下文
