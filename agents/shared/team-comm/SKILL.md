...
```bash
CONTENT='[CC-REPORT] 【进度播报】-【阅即可】 [#'"${ISSUE}"'] 开工：复杂度中等
...
curl -s -X POST http://localhost:9872/api/notify -H 'Content-Type: application/json' \
  -d '{"session":"manager-研发经理","message":"【进度播报】-【阅即可】 [#'"${ISSUE}"'] 开工","type":"success"}' >/dev/null
```

### 编程CC卡住求助

```bash
CONTENT='[CC-REPORT] 【异常发现】-【需回复】 [#'"${ISSUE}"'] 卡15min：Flyway报错
from: cc-wande-play-kimi'"${KIMI_ID}"'-'"${ISSUE}"'
to: manager-研发经理
=============
...
```bash
CONTENT='[CC-REPORT] 【方案评审】-【需回复】 备选队列已更新
from: manager-排程经理
to: manager-研发经理
=============
当前2个CC空位，建议指派：
...
tmux send-keys -t 'manager-研发经理' "$CONTENT" Enter
curl -s -X POST http://localhost:9872/api/notify -H 'Content-Type: application/json' \
  -d '{"session":"manager-研发经理","message":"【方案评审】-【需回复】 备选队列更新，2个空位待指派","type":"info"}' >/dev/null
```
