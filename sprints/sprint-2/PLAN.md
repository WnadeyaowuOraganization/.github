...
> 更新时间：2026-05-02（排程经理第396轮，0 CC活跃，池子完全清空；20个Issue已标Todo待指派；PR#4415/4421/4422/4423/4424已merged）
> ⚠️ needs-prototype冻结、Master Issue不出现在本表
> 注意：本表与GitHub Issue编号一致，无需转换

| # | Issue | 优先级 | 模块 | 说明 | 启动 |
|---|-------|--------|------|------|------|
| 1 | #1619 | P0 | chat | 群信息同步与群成员管理 — 群聊AI配置基础 | ✅ |
| 2 | #1674 | P0 | biz-enablement | 产品参数查询中心API — 搜索/筛选/详情/资料包下载 | ✅ |
| 3 | #1615 | P1 | chat | 按群分角色AI配置 — 多角色模板系统 | ✅ |
| 4 | #1616 | P1 | chat | 每日群聊摘要服务 — 定时+即时双模式 | ✅ |
| 5 | #1617 | P1 | chat | 群聊上下文双层隔离 — 群共享+个人私有分离 | ✅ |
| 6 | #1618 | P1 | chat | @触发与关键词路由 — 群聊精准响应机制 | ✅ |
| 7 | #1878 | P1 | asset-library | 素材统计接口 — 分类数量、标签云、最近上传 | ✅ |
| 8 | #1881 | P1 | asset-library | 素材下载+批量下载+分享链接 | ✅ |
| 9 | #1496 | P2 | backend | NBO行动推荐生成 — Today's Top 3行动 | ✅ |
| 10 | #1497 | P2 | backend | NBO评分引擎 — 项目适配度×意图×赢率综合评分 | ✅ |
| 11 | #1686 | P2 | backend | 报销费控 — 项目费用关联+财务报表导出+供应商联动 | ✅ |
| 12 | #1698 | P2 | backend | 提成绩效 — 提成计算引擎API(阶梯计算+绩效系数+批量) | ✅ |
| 13 | #1538 | P2 | budget | D3造价回填接口 — D3设计完成后自动写入设备科目预算 | ✅ |
| 14 | #1539 | P2 | budget | 历史基准自动积累 — 项目关闭时自动更新成本基准库 | ✅ |
| 15 | #1590 | P2 | budget | 项目成本复盘报告自动生成 | ✅ |
| 16 | #1597 | P2 | budget | 员工借款额度校验Service | ✅ |
| 17 | #1601 | P2 | budget | 报销申请接入预算关卡 | ✅ |
| 18 | #1551 | P2 | wecom | 企微待办API对接 — 审批待办+任务待办推送 | ✅ |
| 19 | #1818 | P2 | brand | 平台定制发布 — 多平台标题/标签/封面微调 | ✅ |
| 20 | #1824 | P2 | brand | 内容创作/编辑页面 — 富文本+多媒体上传+双语编辑 | ✅ |
...
|------|-------|------|--------|------|--------|
| ~~kimi1~~ | ~~#1964~~ | ~~backend~~ | ~~P2~~ | ~~安装程序 — Windows Installer+静默部署~~ | ~~✅ PR#4424已merged，Issue标Done~~ |
| ~~kimi2~~ | ~~#1485~~ | ~~backend~~ | ~~—~~ | ~~新会话（#1665→#1485切换）~~ | ~~✅ PR#4423已merged，Issue标Done~~ |
| ~~kimi3~~ | ~~#1760~~ | ~~frontend~~ | ~~P2~~ | ~~经销模式前端适配 — 项目详情+经销商结算对账~~ | ~~已处理（研发经理kill会话+释放锁）~~ |
| ~~kimi4~~ | ~~#1495~~ | ~~backend~~ | ~~—~~ | ~~新会话（#1659→#1495切换）~~ | ~~✅ PR#4422已merged，Issue标Done~~ |
| ~~kimi5~~ | ~~#1963~~ | ~~backend~~ | ~~P2~~ | ~~Rhino插件对接API — GH参数化模型→PPT效果图自动注入~~ | ~~已处理（研发经理kill会话+释放锁）~~ |

> **0 CC活跃，池子完全清空**；kimi1 PR#4424已merged；kimi2 PR#4423已merged；kimi3 已处理；kimi4 PR#4422已merged；kimi5 已处理；空出5个位置可补派；PR#4415/4421/4422/4423/4424已merged；20个Todo Issue待指派
