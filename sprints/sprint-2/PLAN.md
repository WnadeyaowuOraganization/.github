...
> 更新时间：2026-05-02（排程经理第390轮，4 CC活跃（kimi3已处理）；kimi1 44%🚨(84min,PR#4424,超时43min恶化)；kimi2 42%(154min,PR#4423)；kimi3 已处理；kimi4 40%(154min,PR#4422,3shells活跃)；kimi5 45%🚨(89min,超时42min恶化)；PR#4415已merged；PR#4421已merged；kimi1/kimi5再次通知研发经理）
...
|------|-------|------|--------|------|--------|
| kimi1 | #1964 | backend | P2 | 安装程序 — Windows Installer+静默部署 | 44% 🚨（84min, PR#4424 REVIEW_REQUIRED, **超时43min恶化**） |
| kimi2 | #1485 | backend | — | 新会话（#1665→#1485切换） | 42% 🔥（154min, PR#4423 REVIEW_REQUIRED） |
| ~~kimi3~~ | ~~#1760~~ | ~~frontend~~ | ~~P2~~ | ~~经销模式前端适配 — 项目详情+经销商结算对账~~ | ~~已处理（研发经理kill会话+释放锁）~~ |
| kimi4 | #1495 | backend | — | 新会话（#1659→#1495切换） | 40% 🔥（154min, PR#4422 REVIEW_REQUIRED, 3shells活跃） |
| kimi5 | #1963 | backend | P2 | Rhino插件对接API — GH参数化模型→PPT效果图自动注入 | 45% 🚨（89min, PR#4424 REVIEW_REQUIRED, **超时42min恶化**） |

> 4 CC活跃（kimi3已处理）；kimi1 44%（**超时43min恶化**, 🚨再次通知研发经理）；kimi2 42%（持平, 恢复活跃）；kimi3 已处理；kimi4 40%（持平, 3shells活跃）；kimi5 45%（**超时42min恶化**, 🚨再次通知研发经理）；PR#4415已merged；PR#4421已merged
