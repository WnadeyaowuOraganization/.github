...
> ⏰ 最后更新：2026-04-24 21:20 by 吴耀（D103 样品业务域 B 档攻坚完成 · 5 Tab 原型 + 详设 md + Master #4198 + 22 子 Issue 三重引用回填 · needs-prototype 164→142）
> ⏰ 上次更新：2026-04-24 10:42 by 吴耀（D99 CRM 商务中心 v2.0 原型确认 + 2 遗留点整改 · Master #4021 重开 + Hub 入口页部署 + all-in-one 菜单 +1）
...
| D85a | 04-14 | ✅ | **CC prompt 体系 skill 化落地**（D86~D88 的前置核心事件） | **背景**：原 run-cc.sh 采用**内联长 prompt**（backend/frontend/e2e 各数百行），每次新增规约都要改 prompt 文件+重新派发，难以沉淀。**决策**：把 prompt 中稳定性规约迁移到 `~/projects/.github/docs/agent-docs/skills/` 下的 **skill 体系**（SKILL.md + 子文档），run-cc.sh 仅保留"角色身份+Issue 上下文+入口提示"，规约由 Claude Code 通过描述匹配自动触发。**已落地 skills**：issue-task-md / backend-schema / backend-coding / backend-test / frontend-coding / frontend-e2e / api-contract / menu-contract / pr-visual-proof / pr-body-lint / cc-report / fix-ci-failure / flyway-validate / skill-creator 等 20+。**冒烟验证**：2026-04-14 kimi20 跑 #3630（矿场Phase2 字段对齐）作为 skill 化机制首次整链验证 — PR#3643 顺利 merged（2026-04-14 12:06），证明 skill 描述匹配 / 自动触发 / 多 skill 串联（issue-task-md → backend-schema → backend-coding → backend-test → pr-visual-proof → cc-report）全链打通。**后续治理**：软链分发(D86)、模板矛盾根除(D87)、频繁问题跟踪(D88) 都是本次落地的衍生收尾项。**意义**：CC prompt 从"每次改 run-cc.sh"→"改一处 skill 源全员生效"，并首次实现"经理只下达 Issue、CC 自动编排 skill 链"的新范式 | 吴耀 |
| D86 | 04-14 | ✅ | Skill 软链分发治理 + 双向污染风险方案 1 兜底 | **现状**：run-cc.sh 启动时 `ln -sfn` 把 `~/projects/.github/docs/agent-docs/skills/*` 软链到 `wande-play-kimiN/.claude/skills/`（源一改所有 kimi 生效）。**风险**：软链双向，CC 改 `.claude/skills/<name>/xxx` 会反向污染源。**评估 3 方案**：方案1=git 审计兜底（`.github` 工作树 dirty 即预警）；方案2=源目录只读（chmod，但 skill-creator 需回写破功能）；方案3=run-cc.sh 改 cp -r（牺牲实时生效，但破 skill-creator 设计）。**决策方案 1**：保持软链 + git status 定期检查。**配套**：dev 分支残留 14 个失效软链清理 PR#3670 merged；CLAUDE.md 新增红线 #13 禁 CC 动 `.claude/skills/` 和根 `CLAUDE.md`（运行时资产，untracked 正常）。commit 558a05b | 研发经理 |
...
| D92 | 04-21 | ✅ | Issue 原型支撑全量对账：A 档 34 + B 档 424 + C 档 41（rbac/knowledge/mobile/auth/test/sample/plm/design-ai）处置 | **背景**：执行 wande-prototype v5.1 §5.6 历史清理 SOP，排查全部 750 个 open Issue，91 个已含引用(12%)，659 个不合规。**A 档 34 个回填**：按 biz 域精确匹配 Master+详设 §X.Y — project→执行管理#4043 §2.x / ptc→#4024 / approval→#4023 / crm→#4021 / product-portal→#4045。**A 档 6 个 REJECT**：#2806/2809/2816/2818 去 biz:crm 改加 needs-prototype；#2615 去 biz:cockpit；#1770 保留 biz:rbac 加 needs-prototype。**C 档 41 个试点**：mobile 3 + auth 3 + test 5 + knowledge 10 = 21 close as not planned（其中 test 类永久豁免 needs-prototype，登记于 skill §5.6）；sample 4 加 biz:sample + needs-prototype；plm 3 改标 biz:budget；rbac 11 按标题精确分域（3 asset-library + 1 sample + 4 rbac + 2 hr + 1 wechat #1615）。**新建标签**：`needs-prototype` (#fbca04) / `biz:budget` / `biz:asset-library` / `biz:wechat`。**产出**：`.github/docs/issue-prototype-audit.md` 全量对账报告 / wande-prototype skill 升级 v5.0→v5.1 新增 §5.6 SOP（触发时机+三档处置矩阵+A/B/C 档执行细则）。**剩余**：B 档 424 个无 biz 标签 Issue 按标题关键词归域（待处理）/ C 档剩余 374 个按 biz 域逐个处理（已处理 41，还剩 crm 57 / project 54 / design-ai 39 / d3-parametric 38 / approval 31 / brand-center 31 / wechat 18 / budget 8 / uncategorized 96） | 吴耀 |
...
| D95 | 04-23 | ✅ | biz:crm 三线统一·CRM 商务中心 v2.0 升级·Master #4021 | **背景**：biz:crm 16 open Issue 对账，除 2 个 Master + 3 个代理商工作台冻结 needs-prototype 外，剩 11 个需回填到已有 crm-商务中心原型。**原型 v1.0→v2.0**：查漏补缺 3 个新模块——① 客户 360°详情页（HubSpot/Salesforce Account 360 对标、6 Tab：基本/资产/商机/合同/售后/时间线）/ ② §11 国贸专属页（Tacton CPQ International 对标5 Tab：PI/发货/单据/信用/汇率）/ ③ §12 AI 商务助手（Gong.io/Salesloft 对标、全局右侧常驻组件）。同时合并原独立 `crm-商机详情页` 为 `04-opportunity-detail-full/`（10 Tab）。**交付**：原型 15 新 HTML + 1 合并 10 Tab = 26 文件（02a/7+11/6+ai/2+04-full/11）放入 crm-商务中心/ / 详设.md v1.0+v2.0 合并（+ §4.x/§6 增强/§11/§12章节）/ Master #4021 升级 v2.0 body + 11 子 Issue 清单表 / 3 新子 Issue #4123 客户 360°(P1) + #4124 §11 国贸(P1) + #4125 AI 助手(P2) / 11 子 Issue 三重引用回填（6 商务中心 · #4042/#4037/#4018/#4019/#4017/#4016 + 5 国贸 · #3104→§11.1 / #3106→§11.2 / #3107→§11.3 / #3109→§11.4 / #3111→§11.5）/ all-in-one 商务部菜单 +4 条目（客户 360°/§11/AI/商机详情 10Tab）。**冻结保留**：代理商工作台 3 个 #1754/#2115/#1766 继续 needs-prototype（独立业务，独立规划）。**代码资产**：🟢 4 复用（Opportunity/Payment/ActivityLog/Customer 基础）/ 🟡 3 改造（Customer 360° 聚合/Contacts/Banks）/ 🔴 7 新建（Assets/Aftersales/PI/Shipping/ExportDoc/Credit/FX/AI）——国贸代码 = 0 全新建确认。**合规统计**：A 档合规 326→**340**（+11 回填 +3 新建）/ Master Issue 18→**18**（复用 #4021）/ 原型目录 20→**20**（商机详情页合并减 1，新无）/ biz:crm needs-prototype 8→**3**（仅代理商）/ all-in-one 商务部 +4 菜单。**对标矩阵**：HubSpot Contact 360 + Salesforce Account 360 + Tacton CPQ + Gong.io/Salesloft 四章合一。**下一步**：Sprint-1 排程 #4123 客户 360° + #4019 kehu 迁移、Sprint-2 §11 壳架构 + 5 个 Tab 并行 | 吴耀 |
| D103 | 04-24 | ✅ | biz:sample 22 Issue v5.1 §5.6 B 档补原型（Master #4198 + 样品中心 5 Tab · 6 HTML + 22 子 Issue 三重引用回填） | **背景**：D102 对账成果中 biz:sample 22 open Issue 全部 needs-prototype 冻结，业务域 0 Master 0 原型。sample 业务域于 D26（2026-03-24）提出，D80 dev 分支重建后无历史代码。**决策路径**：Step 1 勾选框回答——架构：Hub 多文件 5 Tab / 深度：Hub 全量 + 5 Tab 全量 / 执行：完整 Step 1-5。confirm_action 批准后执行。**架构**：index.html Hub + 5 Tab = 6 HTML——Tab1 样品全生命周期（5 区域，KPI+筛选表格+D3一键生成+样品箱看板+借还时间线，8 Issue）/ Tab2 色卡材料标准化（3 子Tab，色卡库+材料库+非标审批，3 Issue）/ Tab3 色卡配色器（三栏布局，方案库+配色面板+SVG实时预览+方案对比+总部需求表+审批时间线，4 Issue）/ Tab4 多仓库库存联通（4 区域，KPI+仓库矩阵+智能调拨单+外部仓 API，4 Issue）/ Tab5 样品箱制度（4 区域，KPI+标准箱模板+项目箱一键生成+箱追踪+领用归还，3 Issue）。**代码资产扫描**：🔴 全链路新建（D80 dev 重建后无历史代码，G7e Webhook 未响应当前无法连接，sample 本就为全新业务域）。**SPEC-CARD 自检**：6 HTML × 2 = 12 标记全部通过。**详细设计**：1771 行 42KB，覆盖 §1-§8 全部章节（13 张 MySQL 表 + Flyway 7 迁移脚本 + 14 API 端点 + 18 共享组件 + 58 条验收项）。**交付**：原型 6 HTML（23 KB × 5）+ 详设.md 42 KB，同步到 `.github/docs/design/sample/` 7 文件 / Master Issue **#4198**（[Master][样品管理] 样品管理中心 — 多Tab架构 5Tab）/ 22 子 Issue 三重引用批量回填（原型路径 + 详设 §X.Y + Parent: #4198，幂等关键字 `v5.1 历史回填`，OK 22 / FAIL 0）/ 22 个 needs-prototype 标签全部移除 / all-in-one 综合管理中心菜单 +6 条目（Hub 置顶 ⭐ + 5 Tab 各一条）/ deploy_website 预览 URL：https://www.perplexity.ai/computer/a/mo-de-yang-pin-zhong-xin-yuan-dfsAsrKdQnONWFnWd2inVw。**业务流**：D3 出图→库存匹配→样品申请→支持中心审批（吴严之负责）→工厂制作→入库（标准色卡/材料，非标需审批）→项目箱一键生成（S/M/L/XL 标准尺寸+泡棉卡位）→业务员领用外出→归还入库。**合规统计**：A 档合规 387→**409**（+22 回填）/ Master Issue 20→**21**（新建 #4198）/ 原型目录 22→**23**（新增 sample/）/ biz:sample needs-prototype 22→**0** / all-in-one 综合管理中心菜单从 4 placeholder→**6 条目 + 4 placeholder** / 全库 needs-prototype 164→**142**（精确对齐 -22）/ 含原型引用块 open 377 (65.1%)→**399 (68.9%)** 。**核心教训**：G7e Webhook (54.234.200.59:9800) 在本次无响应，但 sample 为 D80 后新业务域无历史代码，直接按 🔴 标注不阻塞流程——D80 重建后业务域可跳过代码扫描，但需在详设文档明确标注“D80 后新建”。**下一步**：D104 候选——A. biz:asset-library 16 Issue 续补原型（第一批剩余）/ B. biz:budget 13 + biz:project-plan 10 第二批 / C. biz:biz-enablement 18 + biz:data-pipeline 13 第三批 / D. Sprint 排程 sample 业务域 P0 下放开发 | 吴耀 |
| D100 | 04-24 | ✅ | biz:outreach 24 Issue v5.1 §5.6 B 档补原型（Master #4177 + Hub + 5 Phase · 27 HTML + 24 子 Issue 三重引用回填） | **背景**：biz:outreach 24 open Issue （#2412/2413/2414 Phase 0 + #2793/2798/2800/2802 Phase 1 + #2796/2797/2801/2803/2804/2806 Phase 2 + #2807/2808/2809/2810/2811 Phase 3 + #2812/2813/2815 Phase 4 + #2816/2817/2818 Phase 5）全部 needs-prototype 冻结，业务域 0 Master 0 原型。**决策路径**：Step 1 勾选框选 B 档走完整原型流程 + Hub + 5 Phase 多文件架构（与CRM v2.0 Hub 对齐）+ confirm_action 批准后执行。**架构**：Hub (index.html) + 5 Phase 子目录 = 27 HTML——Phase 0 Hub 管控面板 🟢（KPI卡片+服务状态网格+告警列表）/ Phase 1 邮件序列引擎 4 Tab 🔴（Sequences/Templates/Cold Outreach/AB Test，对标 Lemlist+HubSpot Sequences）/ Phase 2 邮件基础设施 6 Tab 🔴（SMTP/Deliverability/Tracking/Rotation/Analytics/Reply Classify，对标 Apollo.io+Mailgun）/ Phase 3 企微营销自动化 5 Tab 🔴（获客链接/渠道活码/自动打标/SOP培育/效果仪表盘，对标 WeSCRM）/ Phase 4 LinkedIn 导入 3 Tab 🔴（批量好友请求/Sales Navigator导入/多通道触达）/ Phase 5 统一获客数据层 3 Tab 🟡（Lead Scoring/ROI仪表盘/CRM交接）。**代码资产扫描**（m7i 54.234.200.59）：🟡 `CrmLeadScoringRuleController.java` 可改造对接 #2816 / 🟡 `CrmLeadController.java` 可改造对接 #2818 / 🟢 pipeline `domestic_clients/` + `international_clients/` + `competitors/` 作 Phase 0 Hub 数据基础 / 🔴 Phase 1-4 全部需新建。**SPEC-CARD 自检**：27 HTML × 2 = 54 标记全部通过。**详细设计**：427 行 20KB，覆盖 §1-§12 全部章节（页面概述 + 5 Phase 章节 §2.1-§2.5 + API 契约 + 数据库设计 + 前端组件清单 + H5 适配 + PageGuide + 验收清单 + Issue 拆分 + 代码资产表）。**交付**：原型 27 HTML（6426 行 HTML）+ 详设.md 427 行，同步到 `.github/docs/design/outreach-marketing/` 28 文件 / Master Issue **#4177** （[Master][外展获客] Hub + 5 Phase 架构、27 页，含 5 Phase 状态表 + 24 子 Issue 清单）/ 24 子 Issue 三重引用批量回填（原型路径 + 详设 §X.Y + Parent: #4177，幂等关键字 `v5.1 历史回填`，OK 24 / FAIL 0）/ 24 个 needs-prototype 标签全部移除 / all-in-one 商务部菜单 +1 条目（⭐ 外展获客中台 v1.0 Hub，插入 CRM Hub 之后）/ deploy_website 预览 URL: https://www.perplexity.ai/computer/a/mo-de-wai-zhan-huo-ke-zhong-ta-oA5nNREjRomQFwPEQ1_.5A 。**对标矩阵**：Lemlist（冷外展）+ HubSpot Sequences（序列）+ Apollo.io（邮件基础设施）+ WeSCRM（企微营销）+ Outreach.io（统一数据层）五家合一。**合规统计**：A 档合规 363→**387**（+24 回填）/ Master Issue 19→**20**（新建 #4177）/ 原型目录 21→**22**（新增 outreach-marketing/）/ biz:outreach needs-prototype 24→**0** / all-in-one 商务部菜单 从 23→**24** 条目。**核心教训**：原型生成时 6 个 website_building 子代理并行分工中 Phase 2 子代理跑偏用了英文简化版 SPEC-CARD（`<!-- SPEC-CARD START/END -->`）而非标准黄色折叠卡模板，批量 Python 脚本修复后通过自检——子代理 objective 中 SPEC-CARD 模板要直接 inline 完整 HTML，不能仅给子代理「请加上标准 SPEC-CARD」模糊指令。**下一步**：D101 候选——A. needs-prototype 继续按业务域补原型（biz:approval 31 / biz:brand-center 31 / biz:design-ai 剩 / biz:wechat 18）/ B. Sprint-4 排程外展获客 P0 Phase 1 邮件序列引擎（#2793/#2798/#2800/#2802）+ Phase 5 Lead Scoring（#2816 可改造已有 CrmLeadScoringRuleController）/ C. wande-auto-code-agent Master 误关修复（系统性修复，D99 遗留） | 吴耀 |
| D99 | 04-24 | ✅ | CRM 商务中心 v2.0 原型确认 + 2 遗留点整改（Master #4021 重开 + Hub 入口页 + all-in-one +1 菜单） | **吴耀确认**：CRM 商务中心 v2.0 原型 × 14 页（36 HTML）全部通过预览审阅。**原型现状**：/home/user/workspace/prototype-crm-v2/ + docs/design/crm-商务中心/ —— v1 基础 10 页 (01-10) 均齐备、v2 新增 4 模块均在位：02a 客户 360°（7 HTML index+6 Tab）/ 04-full 商机详情 10Tab（11 HTML index+10 Tab）/ 11-international 国贸（6 HTML index+5 Tab）/ ai-assistant AI 助手（2 HTML），详设文档 36,944 字节覆盖 §1-§12 全部章节，SPEC-CARD 自检 36 HTML 全部标记数 == 2 通过。**发现两个遗留点**：① Master Issue #4021 被 PR #4119 merge 时 wande-auto-code-agent 误关（2026-04-23 12:13:10Z stateReason=COMPLETED）——但 v2.0 新增 3 子 Issue 仍 open（#4123 AI 助手 P2/#4124 §11 国贸 P1/#4125 客户 360° P1），wande-prototype v5.1 铁律要求 Master 随页面生命周期保持 open；② all-in-one 商务部菜单已有 v2 新增 4 条的各子模块条目（D95 完成），但缺一个统一 Hub 入口（14 页总入口）。**执行整改**：① `gh issue reopen 4021` + 评论说明误关原因和 v2.0 原型状态，state=OPEN stateReason=REOPENED 验证成功；② 创建 prototype-crm-v2/index.html Hub 入口页（197 行/13KB，包含 14 张页面卡片（v1 × 10 + v2 × 4）+ 带 SPEC-CARD + 商机详情 10Tab 已升级标签 + 原型清单统计表格 + Master #4021 状态警告卡片）；deploy_website 预览 URL: https://www.perplexity.ai/computer/a/crm-shang-wu-zhong-xin-v2-0-yu-jNGS9DdPR3mgBLQPOqRqfw；③ 同步复制 index.html 到 .github/docs/design/crm-商务中心/；④ all-in-one 商务部菜单插入置顶条目（位置在矿场之后、Dashboard 之前）：⭐ CRM 商务中心 v2.0 Hub（14 页总入口）。**合规统计**：A 档合规 363 不变 / Master Issue 19 不变（#4021 从误关恢复到 open）/ 原型目录 21 不变（但 crm-商务中心/ 目录增加 index.html，目录从 36 HTML→37 HTML）/ all-in-one 商务部菜单 从 22→23 条目（+1）。**核心教训**：Master Issue 被 auto-close 的 wande-auto-code-agent 健壮行为是系统性问题，需在 wande-org 组织技能新增规则：带 `issue-type:master` 标签的 Issue 即使关联 PR merged，不允许 auto-close。**下一步**：D100 候选——A. needs-prototype 84 冻结按业务域逐个补原型（最有价值）/ B. wande-auto-code-agent Master 误关修复（系统性修复）/ C. Sprint-1 排程 CRM v2.0 三新子 Issue（#4125 客户 360° + #4124 §11 国贸 + #4123 AI 助手） | 吴耀 |
| D98 | 04-24 | ✅ | B 档 18 裸 Issue 按标题关键词归域 + 5 个 D97 遗漏归域（零新原型） | **背景颠覆**：D92 摘要称 B 档 424 个，D97 摘要称 14 个，**实际 D98 摸底总 open 616（非 98）**，无 biz 标签 113 个——其中 11 exempt 豁免 + 84 needs-prototype 冻结（D92 后陆续冻结的 C 档）+ **18 个纯裸 B 档**（真正需要归域）。D97 摘要把 `--limit` 默认 100 截断当成全量是重大误判。**决策路径**：wande-prototype v5.1 §5.6.3 A 档回填式（零风险/最快清理）——按 Issue 标题关键词精准匹配已有 biz 域，批量 `gh issue edit --add-label`。**精准归域映射**（18 裸 B 档）：① 审批引擎系列 10 个 → `biz:approval`（#3153/#3132/#2016/#2020/#2021/#2364/#2365/#2024/#2025/#2027，含 Phase2-22 全系列 + 自动审批规则引擎 + 抄送/SLA/AI预检/菜单权限）；② 矿场 Phase5 国际化 4 个 → `biz:bidding`（#2226/#1486/#2227/#1488，市场流程知识库+采购联盟+跨境评估UI+国际项目数据模型，项目矿场归并 bidding 招投标域）；③ 错误分析中心（驾驶舱）1 个 → `biz:cockpit`（#2275 DORA 指标卡片）；④ 设计 AI 合规检测 1 个 → `biz:design-ai`（#1845 EN1176+ASTM+GB 三标融合）；⑤ D3 v2.0 AI 合规报告 1 个 → `biz:d3-parametric`（#1923 Phase4 PDF 自动生成）；⑥ 资金闭环 1 个 → `biz:finance`（#2358 商机获客成本 Tab）。**同时补归 D97 疏漏 5 个 B 档功能类**：#3622→`biz:project`（全过程资料管理 Master）+ #3185→`biz:project`（全阶段资料完成度看板子）+ #3191→`biz:finance`（甲方资料需求模板库）+ #3154→`biz:approval`（智能路由引擎）+ #3567→`biz:d3-parametric`（电池包管理，保留 needs-prototype 标签）。**执行**：23 次 `gh issue edit --add-label`（5+18 = 23 个 Issue）全部成功，幂等操作无冲突。**合规统计**：总 open 616 不变 / 有 biz 标签 498→**521**（+23，84.6%） / 无 biz 标签 118→**95**（11 exempt + 84 冻结 + **0 裸 B 档**）/ A 档合规 363 不变（B 档归域不涉及回填）/ Master Issue 19 不变 / 原型目录 21 不变 / 新增标签分布：biz:approval +11 / biz:bidding +4 / biz:project +2 / biz:finance +2 / biz:d3-parametric +2 / biz:cockpit +1 / biz:design-ai +1。**wande-prototype v5.1 §5.6.5 登记**：9 个 exempt 豁免理由——6 个 Quick-Fix bugfix（#4095/#4094/#4093/#4063/#4060/#4059 共性：缺 `/api` 前缀/mock 数据/组件路径错，纯补丁无需原型）+ 3 个测试类（#3886 Hook 体系验证/#3570 关系网络Tab/#3558 E2E回归超时）；84 个 needs-prototype 冻结理由——D92 §5.6.3 C 档存量冻结（不进 Sprint，不占 Todo/In Progress，等后续 Sprint 按业务域补原型再解冻）。**核心教训**：后续摸底必须 `--limit 2000` 拉全量，不能用默认 100 截断；D97 摘要传递 `总 open 98` 是传话偏差，实际 616 足足差了 **518 个 Issue** 被遗漏。**下一步**：D99 候选——A. needs-prototype 84 冻结按业务域逐个补原型（最有价值，逐步推进 A 档合规率）/ B. C 档 biz:approval 31 / biz:brand-center 31 补原型 / C. 清理已存在 biz 标签但无原型支撑的子 Issue（继续 §5.6.3 回填） | 吴耀 |
| D97 | 04-23 | ✅ | biz:d3-parametric 3 Issue v5.1 §5.6 B 档补原型（Master #4142 + 7 Tab 平展 + 3 子 Issue 三重引用回填） | **背景**：biz:d3-parametric 实际 3 open（非 D92 摘要的 38，其余早已 close），全部带 needs-prototype，但业务域 0 Master 0 原型。代码资产扫描（m7i 54.234.200.59）揭示 D3 后端 4 Controller + PLM 桥接 1 Controller 齐全（`D3TaskController`/`ProposalD3BridgeController`/`ComponentDependencyGraphController`/`D3ComponentLibraryController` + `PlmD3BridgeController`），前端 8 Vue 但仅 `components/index.vue` 262 行真实实现（Tab2 构件库），其他 7 个全是 7 行骨架。**决策路径**：Step 1 勾选框选 B 档走完整原型流程 + 七 Tab 平展架构（与代码目录 1:1）+ 纯 A 档解冻回填（不新建 Issue）+ PLM/项目中心菜单归属。**架构**：D3 Hub 7 Tab 平展——Tab1 参数化设计（主入口，会话+参数树+GH 插件+BOM 实时统计）/ Tab2 构件库 🟢（6 大分类+多维筛选+批量导入 PLM）/ Tab3 材质标注 🟡（单个/批量/模板+项目配色方案，#2268）/ Tab4 历史案例库 🔴（S3 28 套生产模型+AI 相似度匹配 Top5，#1902+#1922）/ Tab5 效果图渲染 🔴（渲染队列+画廊+推送 AI 中台 #4097）/ Tab6 Web 3D 预览 🔴（Three.js+相机/光照控制+漫游）/ Tab7 BOM 图纸导出 🔴（4 种 BOM 模板+PLM 同步走 `PlmD3BridgeController`）。**对标矩阵**：Tacton CPQ（参数化配置）+ Configura CET（行业 CAD）+ Rhino+Grasshopper（参数引擎）+ SketchFab（3D 预览）。**关键业务伙伴**：PLM #4024（D3 BOM→零件主数据）/ AI 中台 #4097（渲染图→图像工坊）/ 提案引擎（`ProposalD3BridgeController` 提案→D3 会话桥接已实现）/ 超级员工矩阵（`D3TaskController` 任务统计）。**交付**：原型 8 HTML（132KB：index.html 14KB + 7 Tab 13-20KB）+ 详细设计.md 364 行 20KB（§1-§8 完整结构，含 7 Tab 章节+API 契约+4 张 Flyway 新表 SQL+组件清单+H5+PageGuide+验收清单）+ Master Issue #4142（7 Tab 状态表+子 Issue 清单）+ 3 子 Issue 三重引用回填（#2268→§2.3 Tab3 / #1902+#1922→§2.4 Tab4，幂等关键字 `v5.1 历史回填（d3-parametric）`）+ 3 个 needs-prototype 标签全部去除+ all-in-one 项目中心菜单 +1 条目（D3 参数化设计中心）+ deploy_website 预览 URL 已填回。**4 张新表**（Flyway `db/migration_wande_ai/V20260423_*`）：批量材质标注表 + 项目配色方案表 + 历史案例结构化索引表 + AI 相似度匹配缓存表。**合规统计**：A 档合规 360→**363**（+3 回填）/ Master Issue 18→**19**（新建 #4142）/ 原型目录 20→**21**（新增 d3-hub/）/ biz:d3-parametric needs-prototype 3→**0** / all-in-one 项目中心菜单 +1。**下一步**：D98 继续 C 档——biz:approval 31 / biz:brand-center 31；Tab1/2/5/6/7 五个 Tab 子 Issue 在后续 Sprint 单独创建；B 档 424 按标题关键词归域 | 吴耀 |
| D96 | 04-23 | ✅ | biz:project 20 Issue v5.1 §5.6 A 档纯回填（#4043 执行管理 18 + #3994 矿场 1 + #4004 销售记录 1） | **背景**：biz:project 23 open Issue 对账（非 D92 摘要的 54，D93/D94 已清理），3 个 Master 原型全部已存在且完备——⭐ #4043 执行管理（原型 `执行管理/` 10 HTML + 详设 1453 行）+ ⭐ #3994 矿场（原型 `全球项目矿场/` 11 HTML + 详设）+ ⭐ #4004 销售记录（原型 `销售记录体系/prototype/` + `prototype-boss-report/`）。剩 20 个子 Issue 需回填到已有 Master。**决策路径**：Step 1 勾选框选 A 纯回填式（推荐）+ 直接回填到已有 Master — 零新增原型、零新建 Issue，最低成本清理。**精准映射**：① #4043 执行管理 18 个：项目组织管理 18 Phase 14 个（#2524/2526/2527/2528/2529/2530/2531/2532/2533/2534/2535/2536/2537/2539）分布 §2.4/§2.6/§2.7/§2.9/§2.32/§2.33/§2.40、项目 360 看板 6 Phase 3 个（#3179/#3181/#3183）→ §2.34 文档阶段视图·360 看板核心 Tab7 文档中心、AI BOM 推荐 1 个 #2118 → §2.16 BOM 清单 Tab4；② #3994 矿场 1 个：#3999 → §2.9 Tab 区域品类热力图；③ #4004 销售记录 1 个：#4011 → §2.1-§2.4 记录中心四 Tab。**代码资产扫描**（m7i 54.234.200.59）：后端 project 相关模块丰富 6 大域（mine/planning/budget/bidding/project/risk/projectcenter）；前端 `execution/project-hub-360/` 已部分实现（OverviewTab.vue/DocsTab.vue + tab-*.vue 5 个 Tab + bidding/projects/）——🟡 有基础需改造、无需新画原型。**合规统计**：A 档合规 340→**360**（+20 回填）/ Master Issue 18→**18**（复用 3 Master）/ 原型目录 20→**20**（零新增）/ biz:project open（非 Master）20 → **20 全部合规** / all-in-one 菜单 0 变更。**产出**：20 个 Issue body 三重引用追加（原型路径+详设§X.Y+Parent:#N，幂等关键字 `v5.1 历史回填（biz:project）`，OK 20 / SKIP 0 / FAIL 0）/ 无标签变更（无 needs-prototype 标签可去）/ 无 PR 无 deploy。**下一步**：D97 继续 C 档——biz:design-ai 39 / biz:d3-parametric 38 / biz:approval 31 / biz:brand-center 31；B 档 424 按标题关键词归域；Sprint-1 排程 #4123 客户 360° + #4019 kehu 迁移 | 吴耀 |
| D94 | 04-22 | ✅ | biz:collab 整体废弃 + biz:design-ai（AI 内容生成中台）原型启动·Master #4097 | **collab 关闭**：24 个 open Issue 全部 close as not planned（2344/1980/1981/2345/1982/2346/1983/2347/1985/2348/1986/2349/1987/1989/1772/2138/2139/2182/2190/1810/2191/2192/2193/2194），理由：生态位被 design-ai + D66 企微审批贯通 + D67 OA 表单引擎三者叠加覆盖；相关翻译/多语言能力以其他业务原型子功能形式重新规划。**design-ai 架构**：独立中台 1 Hub + 4 Phase——Phase1 图像工坊（5 Tab、对标 Adobe Firefly）/ Phase2 视频工坊（4 Tab、对标 Runway Gen-3）/ Phase3 LoRA 工坊（4 Tab、对标 ComfyUI+Civitai）/ Phase4 方案文本（6 Tab、对标 Writer+Copy.ai）。**代码资产**：🟢 Phase3 LoRA pipeline 8 核心文件完备（wdpp_lora_trainer/preprocessor/validator/pipeline.sh + workflow_config/render_task_service + competitor_crawler/nas_extractor，共 107KB）——Sprint-3 最高 ROI、自建 LoRA-A/B 可降 50%+ AI 成本；🟡 Phase1 pipeline/design_ai 3 collector 已建；🔴 Phase2/Phase4 全新建。**交付**：原型 23 HTML（452KB）/ 详细设计.md（§1-§8盖 23 功能区）11,808 字节 / all-in-one 菜单注册（支持中心下 +5 菜单项 / 同时移除协同修改占位）/ Master Issue #4097 / 23 子 Issue 完成 v5.1 §5.6 A 档三重引用回填（原型路径+详设§X.Y+Parent:#4097）+ 移除 needs-prototype 标签。**合规统计**：A 档合规 303→**326** / Master Issue 17→**18** / 原型目录 19→**20** / biz:collab open 24→**0** / all-in-one 支持中心 +5 菜单。**下一步**：Sprint-3 排程 Phase 3 LoRA 工坊 + Phase 1 图像工坊优先 | 吴耀 |
...
| **数据库+代码重建** | D79：PG→MySQL 8.0。D80：dev分支重建。D81：库名统一`wande-ai`+Pipeline 66个py从PG切MySQL+DB/LLM调用收拢(改配置只改shared/db.py和shared/llm_client.py 2个文件)+vLLM切Token Pool Proxy。D82：Claude Office从自定义Vue组件改为ruoyi-ai内置InnerLink iframe(删除cockpit/claude-office/index.vue，sys_menu path改http URL+component清空+is_frame=0，V3迁移脚本) |
...
| **编程环境架构** | 20个kimi目录独立隔离，各自后端port=7100+N、前端port=8100+N。nginx反向代理(端口8100+N→backend 7100+N)。Flyway自动建表，无手工setup。dev分支清零：仅含ruoyi-ai框架+V2菜单基线+V3 iframe改造(V1__baseline_wande_ai.sql+V2__wande_menu_baseline.sql+V3__claude_office_iframe.sql) |
