...
后端改动提 PR 前必须**按顺序**通过**两道强制门**（缺一 CI 拦截）：

| 顺序 | 门 | 验证层 | 何时 |
|------|---|------|------|
| 1 | **JUnit 单测**（继承 BaseServiceTest） | 编译 + Service 业务分支 / 计算 / 状态机 | **每个后端 Issue 必做**。没有编译通过，Playwright API 也启动不了后端 |
| 2 | **Playwright API spec**（`e2e/tests/backend/**/*.spec.ts`） | HTTP 契约 + 鉴权 + 鉴权头 + DB 持久化端到端 | **每个后端 Issue 必做**（含 Bug 修复 / 纯 CRUD）。覆盖并取代原 curl smoke（能断言 status/body/落库，比 curl 更强）|

**顺序**：JUnit 绿灯→编译通过→后端可启动→Playwright API 才有意义。跳顺序 = 浪费时间 debug 启动失败。

两门证据分别贴 `issues/issue-<N>/task.md`。纯文档 / 配置 Issue 可跳过，要在 task.md 显式注明"跳过原因"。

curl 仍可用作开发期的手动 debug（快速试参数），但**不作 PR 证据门**——Playwright API 已覆盖。
...
## curl 手动 debug（开发期工具，非 PR 证据门）

curl 适合开发时快速试参数、看返回结构、抓 header。**不是** PR 必备证据——Playwright API spec 已覆盖并更强。以下示例仅供联调参考：
...
## Playwright API 测试（**强制**，门 2）

每个后端 Issue 必写至少一个 Playwright API spec 覆盖本 Issue 的核心 endpoint。它取代 curl smoke 作 PR 证据——能断言 status/body/落库，可回归、CI 能跑，而 curl 不能。

前置：JUnit 绿灯 → 编译通过 → 后端可启动 → 才跑 Playwright API。
