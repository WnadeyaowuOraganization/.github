...
|------|-------|------|--------|------|--------|
| kimi1 | #1496 | backend | P2 | NBO行动推荐生成 — Today's Top 3行动 | 31% 🔥（启动后272min） |
| kimi2 | #1497 | backend | P2 | NBO评分引擎 — 项目适配度×意图×赢率综合评分 | 48% 🔥（启动后272min） |
| kimi3 | #1538 | budget | P2 | D3造价回填接口 — D3设计完成后自动写入设备科目预算 | 66% 🔥（启动后272min） |
| kimi4 | #1539 | budget | P2 | 历史基准自动积累 — 项目关闭时自动更新成本基准库 | 43% 🔥（启动后272min） |
| kimi5 | #1590 | budget | P2 | 项目成本复盘报告自动生成 | 40% 🔥（进展39%→40%） |

> **5 CC活跃，池子已满**；kimi1 31%（持平）、kimi2 48%（持平）、kimi3 66%（持平）、kimi4 43%（持平）、kimi5 **40%进展**；PR#4415/4421/4422/4423/4424已merged；PR#4427 mergeState=DIRTY（分支需清理）
