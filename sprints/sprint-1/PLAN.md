...
> 排程经理完成执行管理v2.0全系22个Issue依赖分析（2026-04-16 00:30）
> 批次说明：Wave1已在运行 → Wave2等#3706合并 → Wave3等#3708/#3707合并

| Issue | 优先 | 模块 | 内容 | 启动 | 前置 |
|-------|------|------|------|------|------|
| #3706 | P0 | backend | [执行管理 1/22] 执行项目实体+CRUD+列表API+统计API | ✅（已派kimi1） | — |
| #3708 | P0 | frontend | [执行管理 3/22] 列表页 KPI卡片+筛选+表格（mock优先） | ✅（已派kimi2） | #3706 mock可并行 |
| #3707 | P0 | backend | [执行管理 2/22] 阶段配置API（直销12/经销8/国贸10阶段） | ✅（已派kimi5） | #3706 并行 |
...
| #3725 | P0 | backend | [执行管理 20/22] 验收管理API（5级验收+检查项） | ⏳ #3706合并后可派 | #3706 |
...
| #3588 | P2 | frontend | 产品门户 9/10 H5移动端适配 | ⏳ #3584合并后派 | #3584 |
